...
async fn signup_form(Form(signup): Form<Signup>)-> impl IntoResponse {
	...
	let mut context = Context::new();
	if signup.r#fn.expect("REASON").is_empty(){
		context.insert("fn","يجب كتابة الإسم الأول")
	}
	if signup.ln.expect("REASON").is_empty(){
		context.insert("ln","يجب كتابة الإسم الأخير")
	}
	if signup.un.expect("REASON").is_empty(){
		context.insert("un","يجب كتابة إسم المستخدم")
	}
	if signup.em.expect("REASON").is_empty(){
		context.insert("em","يجب كتابة البريد الإلكتروني")
	}
	if signup.pw.expect("REASON").is_empty(){
		context.insert("pw","يجب كتابة كلمة المرور")
	}
	if signup.rp.expect("REASON").is_empty(){
	...
	if signup.r#fn.expect("REASON").is_empty() && signup.ln.expect("REASON").is_empty() && signup.un.expect("REASON").is_empty() && signup.em.expect("REASON").is_empty() && signup.pw == signup.rp {
