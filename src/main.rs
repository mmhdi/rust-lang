	...
	if signup.r#fn != Some("".to_string()) && signup.ln != Some("".to_string()) && signup.un != Some("".to_string()) && signup.em != Some("".to_string()) && signup.pw != Some("".to_string()) && signup.rp != Some("".to_string()) && signup.pw == signup.rp {
		match db.collection::<Signup>("users").find_one(doc!{"un":&signup.un},None).await.unwrap() {
			...
			_ => match db.collection::<Signup>("users").find_one(doc!{"em":&signup.em},None).await.unwrap() {
				...
				_ => match db.collection("users").insert_one(doc!{"fn":signup.r#fn,"ln":signup.ln,"un":signup.un,"em":signup.em,"pw":signup.pw,"status":"unen","otpem":StdRng::from_entropy().gen_range(1000000..9999999).to_string(),"otpemurl":Alphanumeric.sample_string(&mut StdRng::from_entropy(),16)},None).await.unwrap() {
					Ok(fer) =>{
					_ => match AsyncSmtpTransport::<Tokio1Executor>::starttls_relay("smtp.gmail.com").unwrap().credentials(Credentials::new("mmhdi.me@gmail.com".to_owned(), "zpohzxmbjaxopvnq".to_owned())).build().send(Message::builder().from("NoBody <mmhdi.me@gmail.com>".parse().unwrap()).to("Hei <www.kkyan71@gmail.com>".parse().unwrap()).subject("Happy new async year").body("<h1>Hello World</h1>".to_string()).unwrap()).await.unwrap() {
